...
\begin{itemize}
  \item First stage reuse has no effect on the vehicle launch rate. The first stage production rate is not a rate-limiting step of the launch rate. In this case, operations or second stage production are likely rate-limiting.
  \item First stage reuse increases the launch rate. The first stage production is a rate-limiting step for the vehicle launch rate. Increasing the vehicle first stage life increases the launch rate since first stages do not need to be produced as frequently.
\end{itemize}

For all strategies, we see a decrease in the cost per flight as the number of first stages uses increases. This result is not surprising -- increasing the launch rate decreases the operations cost per flight. It is interesting though to compare the strategy cost per flight trends with the expected cost range for an  expendable vehicle. For the strategies considered in Figure \ref{fig:num_reuse_sweep_LEO_kerosene}, all but the partial parachute recovery strategy show costs per flight below that of an expendable vehicle for a certain minimum number of reuses. With partial parachute recovery, the high-value hardware reuse savings do not compensate for the additional costs of producing and operating a reusable vehicle. However, the other strategies show potential for significant cost per flight savings with a high enough number of first stage reuses. 
...
In order to explore this question, we first define a cost per flight ratio. This will be defined as the ratio between the cost per flight of a vehicle with first stage reuse and the cost per flight of a similar expendable vehicle, and should be in the range [0, 1]. Next we consider the present value of the total savings due to reusability of a launch vehicle over a 20 year timespan with a 20\% discount rate. We evaluate the present value over the full range of cost per flight ratios for selected annual launch rates, and then compare these curves to a credible range of development costs. The results of this analysis are shown in Figure \ref{fig:reuse_npv}. 
...
For low launch rates, it is unlikely that the development costs could be paid off by the cost per flight savings brought by first stage reusability. The investment in development costs loses value faster than any savings from reuse could compensate for it. However, for launch rates greater than 20 launches per year, first stage reusability does appear to be economically viable. With higher launch rates, we see a lower cost per flight ratio since operations costs decrease with increased launch rate. Additionally, higher launch rates mean the investment in the development costs of the reusable vehicle can be paid off sooner, so the development investment will have lost less value. High launch rates are essential for the economic viability of reusable launch vehicles. 
